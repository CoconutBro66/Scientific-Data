...
output_dir = "./finetuned_xlm-roberta-base_multitask_CTF+TAG+LGSD"
max_len = 128
batch_size = 16
...
# 模型定义（含 Conditional Fine CTF+TAG+LGSD）
# ====================
...
class MultiTaskModel(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # ===== LGSD: fine_probs -> type-aware modulation u（d维）=====
        # u = tanh(W_mod p^fine)
        self.loc_gate = torch.nn.Linear(num_fine_labels, self.hidden_size)

        # ===== T3: 定位头（输入为 H_hat = H ⊙ u）=====
    ...
    def forward(
        ...
        **kwargs
        ...
        last_hidden_state = outputs.last_hidden_state  # H, [B, L, d]

        # ===== T1: tri logits / probs =====
        tri_logits = self.tri_classifier(pooler_output)   # z^tgt, [B, 3]
        tri_probs = torch.softmax(tri_logits, dim=-1)     # p^tgt, [B, 3]

        # ===== TAG: h_gate = h_cls ⊙ sigmoid(W_gate p^tgt) =====
        gate = torch.sigmoid(self.gate_layer(tri_probs))  # g, [B, d]
        h_gate = pooler_output * gate                     # [B, d]

        # ===== CTF: c_tgt = W_proj z^tgt =====
        tri_proj_vec = self.tri_proj(tri_logits)          # c^tgt, [B, d]

        # ===== T2: h_fine = [h_gate ; c_tgt] -> fine logits =====
        fine_input = torch.cat([h_gate, tri_proj_vec], dim=-1)  # [B, 2d]
        fine_logits = self.fine_classifier(fine_input)          # z^fine, [B, 12]
        fine_probs = torch.softmax(fine_logits, dim=-1)         # p^fine, [B, 12]

        # ===== LGSD: u = tanh(W_mod p^fine), H_hat = H ⊙ u =====
        fine_cond = torch.tanh(self.loc_gate(fine_probs))       # u, [B, d]
        fine_cond_expanded = fine_cond.unsqueeze(1).expand_as(last_hidden_state)  # [B, L, d]
        conditioned_tokens = last_hidden_state * fine_cond_expanded              # H_hat, [B, L, d]

        # ===== T3: token-level span logits =====
        loc_logits = self.loc_classifier(conditioned_tokens).squeeze(-1)         # [B, L]
...
training_args = TrainingArguments(
    ...
    logging_dir="./logs_multitask_CTF+TAG+LGSD_new",
    logging_steps=100,
...
if __name__ == "__main__":
    ...
    os.makedirs("results/multitask_CTF+TAG+LGSD", exist_ok=True)
    result_path = f"results/multitask_CTF+TAG+LGSD/test_metrics_{training_args.num_train_epochs}_epoch_new.txt"
